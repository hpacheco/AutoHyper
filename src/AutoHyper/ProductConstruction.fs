...
let constructAutomatonSystemProduct
    ...
    let copyToTsMapper = indexList |> List.mapi (fun i j -> j, i) |> Map.ofList
    ...
    let newAps =
        aut.APs |> List.filter (fun (_, i) -> copyToTsMapper.Keys.Contains i |> not)
    ...
    while queue.Count <> 0 do
        ...
        let apEvaluations = tstates |> List.mapi (fun i x -> tslist.[i].ApEval.[x])

        // This map fixes all propositions that refer to a system that take part in the product
        let fixingMap =
            quickApIndexFinderPos
            |> Map.map (fun _ (index, pos) -> Set.contains pos apEvaluations.[index])
...
let constructSelfCompositionAutomaton
    ...
    assert
        (tslist
         ...
         |> List.forall (fun (a, b) -> a |> List.forall (fun x -> List.contains x b)))
    ...
    let combinedAps =
        interestingAps
        |> List.mapi (fun i x -> x |> List.distinct |> List.map (fun c -> c, i))
        |> List.concat
