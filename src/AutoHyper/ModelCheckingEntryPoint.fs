...
let private verify config (tslist: list<TransitionSystem<String>>) (hyperltl: HyperLTL<String>) (m: Mode) =
    ...
    if res then printfn "SAT" else printfn "UNSAT"
...
let explictSystemVerification (config: Configuration) systemPaths propPath m =
    ...
    let hyperltlcontent =
        ...
        with _ ->
            raise <| AutoHyperException $"Could not open/read file %s{propPath}"
    ...
    let hyperltl =
        ...
        | Result.Ok x -> NamedHyperLTL.toHyperLTL x
        | Result.Error err -> raise <| AutoHyperException $"The HyperLTL formula could not be parsed: %s{err}"
    ...
    let tsList =
        if tsList.Length > 1 then
            if tsList.Length <> hyperltl.QuantifierPrefix.Length then
                raise
                <| AutoHyperException "The number of systems given does not match the property"
...
let nuSMVSystemVerification (config: Configuration) systemPaths propPath m =
    ...
    let symbolicHyperLTL =
        ...
        | Result.Ok x -> HyperLTL.SymbolicHyperLTL.NamedSymbolicHyperLTL.toSymbolicHyperLTL x
        | Result.Error err -> raise <| AutoHyperException $"The HyperLTL formula could not be parsed. %s{err}"
    ...
    let unfoldRelationPredicate (a: RelationalAtom) =
        match a with
        | UnaryPred(e, n) -> LTL.Atom((e, n))
        | RelationalEq(e1, n1, e2, n2) ->
            let t1 =
                e1
                |> TransitionSystemLib.SymbolicSystem.Expression.inferType (fun x ->
            ...
            let t =
                ...
                | None ->
                    raise
                    <| AutoHyperException $"Error during unfolding: Could not unify types %A{t1} and %A{t1}."
    ...
    let tsList =
        if plist.Length = 1 then
            let atomList =
                unfoldedHyperLTL.LTLMatrix |> LTL.allAtoms |> Set.map fst |> Set.toList
            ...
            |> List.iter (fun (v: Expression) ->
                ...
                |> Set.iter (fun x ->
                    if
                        (Map.containsKey x plist.[0].VarTypeMap |> not)
                        && (plist.[0].DefineMap.Keys.Contains x |> not)
                    then
                        raise
                        <| AutoHyperException
            ...
            |> List.map (fun i ->
                ...
                |> List.iter (fun (v: Expression) ->
                    ...
                    |> Set.iter (fun x ->
                        if
                            plist.[i].VarTypeMap.ContainsKey x |> not
                            && (plist.[i].DefineMap.ContainsKey x |> not)
                        then
                            raise
                            <| AutoHyperException
...
let booleanProgramVerification (config: Configuration) systemPaths propPath m =
    ...
    let hyperltl =
        ...
        | Result.Error err ->
            raise
            <| AutoHyperException $"The provided HyperLTL formula could not be parsed: %s{err}"
    ...
    let tsList =
        if progList.Length = 1 then
            ...
            let relevantAps =
                hyperltl.LTLMatrix |> LTL.allAtoms |> Set.map (fun (x, _) -> x) |> Set.toList
